...
const NavBar = ({ setUserKind, dark, kind }: { setUserKind: (s: string) => unknown, dark: boolean, kind?: string | null }) => (
  <nav className={`w-screen h-16 hidden sm:flex flex-row fixed px-4 justify-between z-50 ${dark ? 'bg-slate-950' : 'bg-orange-50'}`}>
    ...
    <div className="flex flex-row items-center gap-4">
      ...
      <button onClick={() => setUserKind("consumer")} className={`px-4 hover:underline  underline-offset-2 hover:text-lg transition-all ${kind === "consumer" && 'underline'} duration-200 ${dark ? 'decoration-indigo-500' : 'decoration-orange-400'}`}>Read Reports Easily For Free</button>
      <a href="https://outlook.office365.com/owa/calendar/QuickIntro@forger.ai/bookings/"
        target="_blank"
        referrerPolicy="no-referrer" className="relative">
        <div className={`absolute inset-0 rounded-sm ${dark ? 'bg-gradient-to-r from-indigo-500 to-purple-500' : 'bg-orange-400 hover:bg-orange-500'}`} />
